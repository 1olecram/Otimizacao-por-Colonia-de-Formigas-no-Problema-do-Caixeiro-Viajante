...
def _atualizar_frame(frame_idx, ax, historico_feromonios, G, pos, num_cidades, max_feromonio):
    ax.clear()
    ...
    for i in range(num_cidades):
        for j in range(i + 1, num_cidades):
            ...
            if feromonio > 0.001:  # Ignora arestas com feromônio praticamente zerado
                ...
                # mapear de 0.9 (cinza bem claro) até 0.0 (preto) para que fique visível
...
def gerar_animacao_feromonios(historico_feromonios, num_cidades, arquivo_saida='assets/animacao_feromonios.gif'):
    ...
    historico_feromonios: Lista de matrizes de feromônio ao longo do tempo.
    num_cidades: Quantidade de cidades no grafo.
    arquivo_saida: Caminho onde o arquivo .gif será salvo.
    """
    print(f"\nGerando animação com {len(historico_feromonios)} quadros")
    ...
    anim = animation.FuncAnimation(
        fig, 
        _atualizar_frame, 
        frames=len(historico_feromonios), 
        fargs=(ax, historico_feromonios, G, pos, num_cidades, max_feromonio),
        interval=200, 
        repeat=True
    )
